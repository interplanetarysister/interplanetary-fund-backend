...
## Goal
Make terminal campaign lifecycle propagation and 30-day linked-post retention deterministic, resumable, and safe alongside concurrent automation writers, while propagating terminal listing state within the required 24-hour SLA.
...
5. Make terminal listing propagation idempotent: repeated runs converge on the same state and do not rewrite unchanged records. The authoritative mapping is `monitoredCampaigns.status` → linked `externalPlatforms` listing status: `closed` → `campaign_closed`, `finished` → `campaign_finished`, and `deleted` → `campaign_deleted`. The implementation must propagate each terminal state to every affected linked listing within 24 hours of the authoritative campaign transition.
...
9. Add focused static verification for schema/index names, claim semantics, terminal-state mapping, the 24-hour propagation SLA, 30-day gate, idempotency, and no client-authoritative financial mutations.
10. Validate in Development with overlapping lifecycle/automation runs, duplicate invocations, partial deletion failure, retry, 24-hour propagation timing, and concurrent `distributedPosts` writers before any Production promotion.
...
- Development proves single-winner lifecycle claims, repeat-run convergence, terminal listing propagation within 24 hours, partial-failure recovery, and no duplicate/destructive cross-run behavior.
